...
% constant Q matrix 
Qdiag  = [5e5*ones(1,3),1e7*ones(1,2),1/(dV_max^2)*ones(1,3),1/(dI_max^2),1/(dI_max^2),0];
Q = diag(Qdiag); 
...
Am = diag([-5000,-5000,-5000,-1000,-1000,-1000]);
Ai = [A,zeros(Nstate,Nout+Nstate);-C, zeros(Nout,Nout),C;zeros(Nstate,Nout+Nstate) Am]; 
Bi = [B;zeros(Nout+Nstate,Nin)];
% Ci = eye(size(Ai)); 
% Di = D;
Qdiag  = [1/(dV_max^2)*ones(1,3),1/(dI_max^2)*ones(1,2),0,5e6*ones(1,3),5e6*ones(1,2),1/(dV_max^2)*ones(1,3),1/(dI_max^2)*ones(1,2),0];
Q = diag(Qdiag);
...
hold on; plot(phi,K34);
title('D Dependencies on Ia');
% legend('V1->D1','V2->D1','V3->D1');
legend('I1->D1','I1->D2','I1->D3');
subplot(3,1,2);
plot(phi,K44); 
hold on; plot(phi,K54);
hold on; plot(phi,K64);
% legend('Ia->D1','Ib->D1')
legend('I1->D4','I1->D5','I1->D6');
...
% hold on; plot(phi,zeros(1,length(phi)));
save ('Knom_dq','K_log','xnom_log','phi'); 
