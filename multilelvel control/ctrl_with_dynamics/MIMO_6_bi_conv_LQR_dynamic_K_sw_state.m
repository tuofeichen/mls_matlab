...
%  flag for only trying to read it once
K13_flag = 1; 
K12_flag = 0; 
...
% decision on what set of gains to use
if (abs(Igd(3))<1) && (K13_flag == 0)
    load Knom13.mat
    K13_flag = 1; 
    K12_flag = 0; 
end

if (abs(Igd(3))>1) && (K12_flag == 0)
    load Knom12.mat
    K13_flag = 0; 
    K12_flag = 1; 
end
...
state   =   x - xnom; 
err     =   yd - y;                 % only care about absolute value 
