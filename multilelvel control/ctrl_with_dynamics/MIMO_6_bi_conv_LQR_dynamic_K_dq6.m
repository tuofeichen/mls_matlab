...
T              =   Kpark*Kclark;
x_dq           =   [x(1:3);T*x(4:end)]; % DQ transformation
y_dq           =   x_dq(1:5); 
y_ddq          =   [45, 50, 55, Id_amp, Iq_amp]';
tnom_step      =   ceil(interp1(phi,tlen,theta,'nearest','extrap'));
x_nom          =   [ xnom_log(tnom_step,1:3)';Id_amp;Iq_amp;0]; 
...
ydq_log(:,ii) = [Vd,Id_amp,Iq_amp]'; 
xd_log(:,ii) = [xnom_log(tnom_step,1:3),Igd]';
