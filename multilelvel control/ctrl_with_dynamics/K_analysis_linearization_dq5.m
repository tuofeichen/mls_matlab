...
% constant Q matrix 
Qdiag  = [5e5*ones(1,3),5e6*ones(1,2),1/(Vpv^2)*ones(1,3),1/(Id_max^2),1/(Id_max^2)];
Q = diag(Qdiag); 
...
Aiv = [-D(1),-D(3),0; 0 -D(4) -D(5); -D(2) 0 -D(6)]*Tinv(:,1:2)/Cin;
Avi = T(1:2,:)*[D(1) 0 D(2); D(3) D(4) 0 ; 0 D(5) D(6)]/Lout;
A = [zeros(3,3), Aiv; Avi,[0,60*2*pi;-60*2*pi,0]]; %additional term related to the DQ coupling 

%A = [zeros(3,3), Aiv; Avi,zeros(2,2)];
Bv = [-Ig(1) 0 -Ig(2) 0 0 0; 
     0 0 0 -Ig(2) -Ig(3) 0; 
     0 -Ig(1) 0 0 0 -Ig(3)]/Cin;
Bi = T(1:end-1,:)*[Vpv Vpv 0   0   0  0; 
       0  0  Vpv Vpv  0  0;
       0  0  0   0  Vpv Vpv]/Lout;
   
   

Babc_v = [-Ig(1) 0 -Ig(2) 0 0 0; 
     0 0 0 -Ig(2) -Ig(3) 0; 
     0 -Ig(1) 0 0 0 -Ig(3)]/Cin;
Babc_i = [Vpv Vpv 0   0   0  0; 
       0  0  Vpv Vpv  0  0;
       0  0  0   0  Vpv Vpv]/Lout;
