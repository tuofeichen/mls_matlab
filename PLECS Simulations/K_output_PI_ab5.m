...
Nout   = 6;   % Number of outputs (last output is not used)
...
Avn = [-(D(1)+D(3))/3,-(D(4)+D(5))/3,-(D(2)+D(6))/3;...
       -(D(1)+D(3))/3,-(D(4)+D(5))/3,-(D(2)+D(6))/3;...
       -(D(1)+D(3))/3,-(D(4)+D(5))/3,-(D(2)+D(6))/3;]/Lout;
Avi = Avi+ Avn;    
...
Bin = [-Vpv(1),-Vpv(3),-Vpv(1),-Vpv(2),-Vpv(2),-Vpv(3);
       -Vpv(1),-Vpv(3),-Vpv(1),-Vpv(2),-Vpv(2),-Vpv(3);
       -Vpv(1),-Vpv(3),-Vpv(1),-Vpv(2),-Vpv(2),-Vpv(3);]/3/Lout;
...
Kabc = [K1_log(ii,id_vi),0,0,  K1_log(ii,id_ii),0,0,  K1_log(ii,id_vp),0,0,  K1_log(ii,id_ip),0,0;
        0,0,K2_log(ii,id_vi),  K2_log(ii,id_ii),0,0,  0,0,K2_log(ii,id_vp),  K2_log(ii,id_ip),0,0;
        K3_log(ii,id_vi),0,0,  0,K3_log(ii,id_ii),0,  K3_log(ii,id_vp),0,0,  0,K3_log(ii,id_ip),0;
        0,K4_log(ii,id_vi),0,  0,K4_log(ii,id_ii),0,  0,K4_log(ii,id_vp),0,  0,K4_log(ii,id_ip),0;
        0,K5_log(ii,id_vi),0,  0,0,K5_log(ii,id_ii),  0,K5_log(ii,id_vp),0,  0,0,K5_log(ii,id_ip);
        0,0,K6_log(ii,id_vi),  0,0,K6_log(ii,id_ii),  0,0,K6_log(ii,id_vp),  0,0,K6_log(ii,id_ip);
        ];
    
[V,lambda]  = eig(Ai-Bi*Kabc); 
P = diag(lambda); 
